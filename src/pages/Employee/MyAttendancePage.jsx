...
import { LogIn, LogOut, Home, Building2, Download, CheckCircle, Calendar, BarChart3, Clock, ChevronLeft, ChevronRight } from "lucide-react";
...
const AttendanceAPI = {
  ...
  checkOut: (id) => apiClient.patch(`/attendance/checkout/${id}`, {
...
export default function MyAttendancePage() {
  ...
  // ✅ Extract empId from user.identifier (preserved from original)
  ...
  // ✅ NEW: Month/year + pagination state
  ...
  // ✅ DEBUG: Log user data (preserved from original)
  useEffect(() => {
  ...
  // ✅ NEW: Helper to check if date matches selected month/year
  ...
  // Main load logic (preserved from original)
  const load = async () => {
    if (!empId) {
    ...
    try {
      ...
      const [attendanceData, leaveResponse] = await Promise.all([
        ...
        apiClient.get(`/leave-approvel/employee/${empId}`).catch(err => {
          console.error("Failed to load leave data:", err?.response?.data || err);
          return { data: [] };
        })
      ]);
      ...
      // APPROVED LEAVE logic for disable
      ...
      if (err.response) console.error('❌ Load error response:', err.response.data);
    } finally {
      setLoading(false);
    }
  };
  ...
  // Check-In handler (preserved from original)
  const handleCheckIn = async () => {
    if (isOnLeaveToday)
      return alert(
        "Check-in disabled: You are on approved leave today."
      );
    if (todayHoliday)
      return alert(
    ...
    try {
      ...
      // Replace an "Absent" record with new check-in if present
  ...
  // Check-Out handler (preserved from original)
  const handleCheckOut = async () => {
    if (isOnLeaveToday)
      return alert(
        "Check-out disabled: You are on approved leave today."
      );
    if (todayHoliday)
      return alert(
    ...
    try {
      ...
      try {
        toast && toast({ title: 'Checked out', description: 'You will be logged out now.' });
      } catch (e) {
      ...
      try { window.__autoLogoutTimeout = _t; } catch (e) {}
    } catch (err) {
  ...
  // Check-In / Check-Out enable states (preserved from original)
  ...
  // Sort attendance by date descending (preserved from original)
  const sortedRecords = useMemo(() => [...records].sort((a, b) => new Date(b.date) - new Date(a.date)), [records]);

  // ✅ NEW: Monthly filtered records (only with check-in)
  ...
  // ✅ UPDATED: Attendance summary based on monthly records with stricter present count
  const summary = useMemo(() => {
    const total = monthlyRecords.length;
    const present = monthlyRecords.filter((r) => r.checkIn && r.checkOut).length; // ✅ Both required
    ...
    return { total, present, leave, absent, rate, totalHoursDisplay: `${hours}h${minutes > 0 ? ` ${minutes}m` : ""}` };
  }, [monthlyRecords]);



  // ✅ UPDATED: Download CSV for current month
  const downloadCSV = () => {
    if (!monthlyRecords.length) return alert("No records to download");
  ...
  // ✅ UPDATED: Download PDF for current month
  const downloadPDF = () => {
    if (!monthlyRecords.length) return alert("No records to download");
    const doc = new jsPDF();
  ...
  // Show loading/error state if no empId (preserved from original)
  if (!empId) {
    return (
      <div className="p-6 flex items-center justify-center min-h-screen bg-gray-50">
        <Card className="max-w-md">
          <CardContent className="p-6 text-center">
            <h2 className="text-xl font-bold mb-2">No Employee ID Found</h2>
            <p className="text-gray-600 mb-4">Please ensure you are logged in correctly.</p>
    ...
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">Logged in as {empName} ({empId})</p>
        </div>
      </div>
      ...
      {/* ✅ UPDATED: ATTENDANCE TABLE - now shows paginated monthly records */}
      <Card className="mt-4">
        <CardHeader>
          <CardTitle>Attendance History</CardTitle>
          <CardDescription>Current month records (checked-in days only)</CardDescription>
        </CardHeader>
        <CardContent>
